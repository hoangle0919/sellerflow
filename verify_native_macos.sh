...
say "10. Writing report"
{
  ...
  echo
  echo "Generated by \`verify_native_macos.sh\`."
  ...
  echo "requires >= 3.11)."
  echo
  echo "> Sanitized by design: no usernames, absolute paths, credentials or"
  echo "> device identifiers. This file is safe to commit. See \`evidence/README.md\`."
  echo
  echo "- **Date:** $(date -u '+%Y-%m-%d') (UTC)"
  echo "- **OS:** macOS $(sw_vers -productVersion 2>/dev/null || echo 'unknown') ($(uname -s) $(uname -r))"
  echo "- **Architecture:** $(uname -m)"
  echo "- **Branch:** \`$(git -C "$REPO" branch --show-current)\`"
  echo "- **Commit:** \`$(git -C "$REPO" rev-parse HEAD)\`"
  echo "- **Interpreter:** $(python --version 2>&1)"
  echo "- **Environment:** virtualenv inside the external-SSD working copy"
  echo
...
PY
  ...
  echo
  if [ "$fail" -eq 0 ]; then
    echo '**PASS** — the pinned environment installs and the supported stack is verified natively.'
  else
...
# stays committable even if a future edit reintroduces one.
if command -v sed >/dev/null 2>&1; then
  sed -i '' -e "s#${REPO}#<repo>#g" -e "s#/Users/[^ \`\"]*#<path>#g" "$REPORT" 2>/dev/null || true
fi
