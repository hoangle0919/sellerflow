...
- **Reproducibility, at the strength the measurement supports.** All five artifacts reproduce numerically at published precision on every platform tested. Byte equality is **platform-dependent and not claimed across platforms**: 5/5 byte-identical on Linux/aarch64 CPython 3.10.12. macOS has not been re-measured for the current generation and no count is quoted; the superseded generation's figures are retained in the registry, labelled as measurements of those files. An earlier unqualified byte-identity claim rested on a step that re-hashed the committed file instead of regenerating it, and is withdrawn.
- **`validation_v2` is canonicalized** (D-038, regenerated under A-9) — SHA-256 `4f26f04e…`. Its predecessor `validation_v1` is retained unchanged.
