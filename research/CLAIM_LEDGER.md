...
|---|---|---|---|---|---|---|
| **P-1** | sensitivity_result | `f* = 1.0945` is the **nearest point on the registered 0.0005-step grid** on the reference path to the illustrative 18%/12-month amortizing reference: **19.537656%** against **19.561817%**, a residual of **≈ 0.02416 percentage points**. | `validation_v2` → `/pricing/equal_cost/{f_star,apr}`, `/pricing/benchmark_b_apr` | `7d9b9d0f…` | **Not an exact match** — it is the closest point on the swept cap-factor grid, and the residual must be stated wherever the figure is. **The residual is a grid-resolution result, not a limit on attainability (D-056):** APR is piecewise continuous in `f`, and a **numerical** reference-path root sits at approximately `f = 1.09462066267694`. Do **not** write that duration integrality makes an exact match unattainable. Say **"reference-path cost-matched"**, never "equal cost": `f*` was solved on one flat, shock-free path, and on simulated paths the realised rate differs because duration moves with revenue | ~~"equal-effective-cost cap"~~, and any wording implying exact cost equality |
