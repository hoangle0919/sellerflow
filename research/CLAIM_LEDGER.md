...
| **M-7** | mathematical_property | Incomplete recovery arises from exactly four mechanisms: zero revenue; maturity or terminal write-off; a binding finite horizon; **or strictly positive revenue whose infinite sum converges below `f·A/r`**. | `DERIVATIONS.md` P7 | The fourth is the non-obvious one and must not be dropped: revenue positive in every period forever is **not** sufficient for completion. Bounded-away-from-zero is sufficient; merely positive is not | paper, appendix |
