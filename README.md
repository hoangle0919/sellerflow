...
- **Risk model:** Random Forest + Logistic Regression ensemble trained on **synthetic labels**. A **secondary demonstration component with no measured predictive validity** — not a validated default predictor. **The ensemble is also optional at runtime:** if the model artifact fails to build or load on a deploy, scoring falls back to a deterministic heuristic, and `GET /api/health` reports the active path (`scoring_path`) and `sklearn_runtime`. **The active scoring path changes the result.** The financing formulas are deterministic *once a risk tier is supplied*, but the scorer is what assigns that tier — and the two paths disagree on the tier for a majority of profiles. Every downstream term keys off the tier, so the advance, remittance rate, cap factor, repayment amounts and eligibility all move with it: at 200M monthly revenue the cap is 414,000,000 at Low Risk against 249,600,000 at Medium, and High Risk zeroes the advance outright. An earlier version of this paragraph said the numbers were "identical either way"; that was wrong and is retracted (see `research/CLAIM_LEDGER.md`). See [Model status](#model-status).
