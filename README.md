...
RBF analyzes ecommerce-merchant revenue, tests repayment scenarios, and surfaces the risks and assumptions behind every financing recommendation. The motivating premise is that many small Vietnamese e-commerce sellers have limited or no credit-bureau (CIC) file, while their platform activity — revenue, growth, returns, fulfilment history — is recorded continuously. **No prevalence figure is claimed** — this project holds no such data — and the comparison to a bureau file is a motivation for the design, **not a measured finding**. RBF turns **merchant-submitted operational data** into a structured financing analysis: recommended advance, remittance percentage, repayment cap, downside scenarios, and the categorized risk findings behind the recommendation. **Direct platform integration is design intent, not shipped** — the prototype connects to no platform API, so the figures it works from are self-reported and unverified.

> **Live demonstration:** https://sellerflow-production.up.railway.app
>
> The live demo runs **this repository's current build**. It is a **demonstration, not a lending service**: it holds no capital and makes no real credit decisions. **As of August 2026, no external merchant data were used in this study; the public deployment is a demonstration, not a lending service** — the database holds demonstration records and developer tests only, so there is no third-party personal information. Its risk score comes from whichever scorer is active — the RF+LR ensemble when its artifact loads, otherwise a deterministic heuristic fallback. The ensemble path is trained on synthetic data with a circular label; neither path has measured predictive validity. See [Model status](#model-status).
>
...
The risk score comes from **whichever scorer is active** and is an **unvalidated demo score, not a probability of default**. When the model artifact loads, that is the RF+LR ensemble, whose synthetic training label was generated by a formula over the same ten features (D-001); when it does not, a deterministic heuristic runs instead. **Neither path has measured predictive validity**, and the two do not read the same inputs, so they can assign different risk tiers to the same merchant. Everything downstream of it — the advance amount, remittance percentage, repayment cap, scenario durations, and risk categorization — is **plain deterministic arithmetic in `backend/financing_engine.py`, with no model or AI call anywhere in the path.** That split is deliberate: a language model may explain a number in this product; it never computes one.

- **Risk model:** Random Forest + Logistic Regression ensemble trained on **synthetic labels**. A **secondary demonstration component with no measured predictive validity** — not a validated default predictor. **The ensemble is also optional at runtime:** if the model artifact fails to build or load on a deploy, scoring falls back to a deterministic heuristic, and `GET /api/health` reports the active path (`scoring_path`) and `sklearn_runtime`. The live demo may therefore be running the heuristic rather than the ensemble. **The active scoring path changes the result.** The financing formulas are deterministic *once a risk tier is supplied*, but the scorer is what assigns that tier — and the two paths disagree on the tier for a majority of profiles. Every downstream term keys off the tier: `scoring_path → pd_score → risk tier → advance %, remittance %, cap factor, and whether financing is offered at all`. At 200M monthly revenue the cap is 414,000,000 at Low Risk against 249,600,000 at Medium, and High Risk zeroes the advance outright. An earlier version of this paragraph said the numbers were "identical either way"; that was wrong and is retracted (see `research/CLAIM_LEDGER.md` §6). See [Model status](#model-status).
- **Financing structure:** advance = 15% of annual revenue (low risk) or 8% (medium risk); repayment cap = advance × 1.15–1.30; remittance = 8–12% of monthly revenue. Because remittance is a percentage of revenue rather than a fixed installment, a revenue decline lengthens the repayment term rather than raising the payment. That is the mechanical point of the scenario analysis below. This is **not a claim of** default prevention. Permanent zero revenue occurring **before completion, while a contractual balance is still outstanding**, can prevent the cap from ever being reached. Timing decides it: in the simulated closure scenarios at the illustrative `f = 1.20`, closure from month 7 leaves **100.0%** of paths incomplete, closure from month 13 leaves **76.2%**, and a temporary three-month closure leaves only **2.0%** (0.0% at `f* = 1.0945`). In the scenarios where the term extends, that extension is the provider's cost; the direction is not universal — P4 makes recovery ordering conditional on the realised path, and both directions occur in the registered scenarios. The failure case is real. Both are stated explicitly rather than left implicit in a chart.
...
Tests use hand-computed expected values, not just "it ran without error". The `financing_engine.py` suite covers: revenue-metric statistics with and without history, financing-structure math across risk tiers and requested-vs-recommended amounts, scenario-duration ordering under decline and growth, zero/edge-case inputs, risk-finding severity and categorization, and end-to-end completeness scoring. The `integrity_engine.py` suite covers: each check firing on a known-bad profile and staying quiet on a clean one, severity assignment, and resubmission-divergence across submissions. The FastAPI routes, the Simulation Lab surface, the scoring-path disclosure and the public-copy tripwires are covered by `test_api.py`, `test_lab.py`, `test_scoring_path_disclosure.py` and `test_public_copy.py`. What remains uncovered is the ML ensemble's training pipeline and end-to-end browser behaviour outside the nine Playwright checks — see [Known gaps](#known-gaps).
...
| **Unify credit + fraud** | One assessment returns both a credit view and a rule-based **integrity screen** (5 misrepresentation/consistency checks); an elevated integrity level **marks the demonstration assessment for review** alongside the integrity findings. | Supervised fraud/AML model on device, behavioral, and network signals — not just self-reported fields. |
