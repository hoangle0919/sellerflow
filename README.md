...
- **Risk model:** Random Forest + Logistic Regression ensemble trained on **synthetic labels**. A **secondary demonstration component with no measured predictive validity** — not a validated default predictor. **The ensemble is also optional at runtime:** if the model artifact fails to build or load on a deploy, scoring falls back to a deterministic heuristic, and `GET /api/health` reports the active path (`scoring_path`). The live demo may therefore be running the heuristic rather than the ensemble. **The financing formulas are deterministic once a risk tier is supplied; however, the active scoring path can change the assigned tier and therefore the displayed advance, remittance rate, cap factor, and repayment amounts.** The two paths are not output-equivalent: `scoring_path → pd_score → risk tier → advance %, remittance %, cap factor, and whether financing is offered at all`. See [Model status](#model-status).
