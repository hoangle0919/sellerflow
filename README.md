...
- **Financing structure:** advance = 15% of annual revenue (low risk) or 8% (medium risk); repayment cap = advance × 1.15–1.30; remittance = 8–12% of monthly revenue. Because remittance is a percentage of revenue rather than a fixed installment, a revenue decline lengthens the repayment term rather than raising the payment. That is the mechanical point of the scenario analysis below. This is **not a claim of** default prevention: where revenue reaches zero the cap is simply never reached, and in the simulated closure scenarios incomplete recovery runs to 100%. The extension is the provider's cost, and the failure case is real — both are stated explicitly rather than left implicit in a chart.
...
- Null and unfavourable results are reported, not dropped — including a guardrail design that provably never activates, and a provider-recovery effect that reverses sign under non-declining revenue (traced to integer rounding in the matching rule, not economics).
